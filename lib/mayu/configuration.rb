#!/usr/bin/env ruby -rbundler/setup
# frozen_string_literal: true

# Copyright Andreas Alin <andreas.alin@gmail.com>
# License: AGPL-3.0

require "toml"

module Mayu
  module Configuration
    class ConfigNotFound < StandardError
    end

    def self.convert_env(value)
      if var = value[/\A\$(.*)/, 1]
        ENV[var]
      else
        value
      end
    end

    Config = Data.define(:root, :secret_key, :server, :metrics) do
      def self.parse(root, config)
        new(
          root:,
          secret_key: Configuration.convert_env(config.fetch("secret_key")),
          server: ServerConfig.parse(config.fetch("server")),
          metrics: MetricsConfig.parse(config.fetch("metrics")),
        )
      end
    end

    ServerConfig = Data.define(
      :listen,
      :hmr?,
      :render_exceptions?,
      :self_signed_cert?,
      :generate_assets?,
    ) do
      def self.parse(config)
        new(
          listen: Configuration.convert_env(config.fetch("listen", "https://localhost:9292")),
          hmr?: config.fetch("hmr", false),
          render_exceptions?: config.fetch("render_exceptions", false),
          self_signed_cert?: config.fetch("self_signed_cert", false),
          generate_assets?: config.fetch("self_signed_cert", false),
        )
      end
    end

    MetricsConfig = Data.define(:enabled?, :listen) do
      def self.parse(config)
        new(
          enabled?: config.fetch("enabled", true),
          listen: config.fetch("listen", "http://localhost:9293"),
        )
      end
    end

    def self.load(path)
      path
        .then { File.read(_1) }
        .then { TOML.load(_1) }
        .map do
          [_1.to_sym, Config.parse(File.dirname(path), _2)]
        end
        .to_h
    end

    def self.find(filename = "mayu.toml", dir = Dir.pwd)
      path = File.join(dir, filename)

      if File.exist?(path)
        path
      else
        parent = File.join(dir, "..")
        return if dir == parent
        find_config_file(filename, parent)
      end
    end

    def self.with(&)
      path = find

      unless path
        raise ConfigNotFound, "Could not find mayu.toml in #{Dir.pwd}"
      end

      config = self.load(path)

      Dir.chdir(File.dirname(path)) do
        yield config
      end
    end
  end
end
